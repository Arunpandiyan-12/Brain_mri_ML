...
class BrainTumorModel(nn.Module):
    ...
    def _find_gradcam_conv(self) -> nn.Conv2d:
        ...
        if self._backbone_type == "timm_efficientnet":
            # Prefer earlier block for better spatial resolution
...
class InferencePipeline:
    ...
    case_id:           str = "case",
        ...
        probs_np = _out["probs"][0].cpu().numpy()
        top2 = probs_np.argsort()[-2:]  # ascending, so top2[-1] = highest prob
        ...
        for cls in top2:
            ...
            heatmaps.append((cls, heatmap))

        # Save overlays + raw heatmap for top-1 class
        gradcam_paths = []
        heatmap_path  = None
        ...
        for i, (cls, heatmap) in enumerate(heatmaps):
            ...
            overlay_path = f"{output_dir}/{case_id}_gradcam_class_{cls}.jpg"
            cv2.imwrite(overlay_path, overlay)
            gradcam_paths.append(overlay_path)

            # top2[-1] is the highest-prob class → save its raw heatmap
            if i == len(heatmaps) - 1:
        ...
        return {
            ...
            "gradcam_paths":       gradcam_paths,       # list of top-2 overlays
            "gradcam_path":        gradcam_paths[-1],   # top-1 blended overlay
            "heatmap_path":        heatmap_path,         # top-1 raw heatmap
